...
object A_OpaqueTypesRuntime:
  ...
  private[advanced] object DNI:
    private inline def isValidDNI(number: Int, letter: String, expected: String): DNI =
      ...
      then new DNI(number, letter)
        ...
        error(
          "DNI number '" + constValue[ToString[
            number.type
          ]] + "' does not match the control letter '" + letter + "', expected '" + expected + "'"
        )
    ...
    inline def apply(dniNumber: Int, controlLetter: String): DNI =
      ...
      then
        error(
          "Invalid DNI number '" + constValue[ToString[dniNumber.type]] + "' should be positive and contain 8 digits"
        )
  ...
  private[advanced] opaque type NIE = (String, Int, String)
  extension (nie: NIE) def formatted: String = s"${nie._1}-${s"%0${7}d".format(nie._2)}-${nie._3}"
  private[advanced] object NIE:
    private inline def isValidNIE(
      ...
      then new NIE(nieLetter, nieNumber, controlLetter)
        ...
        error(
          "NIE number '" + constValue[ToString[
            fullNumber.type
          ]] + "' does not match the control letter '" + controlLetter + "', expected '" + expectedLetter + "'"
        )
    ...
    inline def apply(nieLetter: String, nieNumber: Int, controlLetter: String): NIE =
      ...
      then
        error(
          "Invalid NIE Number: '" + constValue[ToString[nieNumber.type]] + "' should be positive and contain 7 digits"
        )
      else inline if constValue[![Matches[controlLetter.type, "[TRWAGMYFPDXBNJZSQVHLCKE]{1}"]]]
      then error("Invalid ControlLetter: '" + controlLetter + "' is not a valid Control letter")
      else
        inline nieLetter match
          case "X" =>
            inline (nieNumber % 23) match
          ...
          case "Y" =>
            inline ((10000000 + nieNumber) % 23) match
          ...
          case "Z" =>
            inline ((20000000 + nieNumber) % 23) match

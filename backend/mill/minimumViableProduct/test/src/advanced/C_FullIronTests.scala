...
import advanced.C_FullIron.*
import mvp.common.*
import utest.*

object C_FullIronTests extends TestSuite:
    ...
    test("DNI"):
      ...
      test("Compile positives"):
        Seq(
          ...
          case (input, expected) =>
            DNI
              .either(ValidInput.applyUnsafe(input))
              .foreach: result =>
                assert(result.formatted == expected)

      test("Invalid Number"):
        DNI.either(ValidInput.applyUnsafe("1234567AT")) match
          case Left(error) => assert(error == "Should contain 8 digits")
          case Right(_)    => assert(false)

      test("Invalid ControlLetter"):
        DNI.either(ValidInput.applyUnsafe("12345678Ñ")) match
          case Left(error) => assert(error == InvalidControlLetter("Ñ").cause)
          case Right(_)    => assert(false)

      test("Invalid Dni"):
        DNI.either(ValidInput.applyUnsafe("00000001Z")) match

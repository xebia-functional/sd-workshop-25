...
object Rules:
  ...
  object messages:
    ...
    def invalidNumber(number: String): String =
      s"Invalid Number: '$number' should not contain letters"
    def invalidDniNumber(dniNumber: String): String =
      s"Invalid DNI Number: '$dniNumber' should contain 8 digits"
    def invalidNieNumber(nieNumber: String): String =
      s"Invalid NIE Number: '$nieNumber' should contain 7 digits"
  ...
  object requirements:
    ...
    def requireValidInput(input: String): Unit =
      require(
        input.length == 9 && input.forall(_.isLetterOrDigit),
        invalidInput(input)
      )
    ...
    def requireValidControlLetter(controlLetter: String): Unit =
      require(
        ControlLetter.values.map(_.toString).contains(controlLetter),
        invalidControlLetter(controlLetter)
      )
    def requireValidDni(dniNumber: String, letter: ControlLetter): Unit =
      require(
        dniNumber.toInt % 23 == letter.ordinal,
        invalidDni(dniNumber, letter)
      )
    def requireValidNie(
        nieLetter: NieLetter,
        nieNumber: String,
